module ContactApp.Handlers
open System.Threading.Tasks
open ContactApp.templates
open ContactApp.Models
open ContactApp.Tools
open Microsoft.AspNetCore.Http
open Oxpecker
open Oxpecker.Htmx

let mutable archiver = Archiver(ResizeArray())

let getContacts: EndpointHandler =
    fun ctx ->
        task {
            let page = ctx.TryGetQueryValue "page" |> Option.map int |> Option.defaultValue 1
            match ctx.TryGetQueryValue "q" with
            | Some search ->
                let! result = ContactService.searchContact search
                let resultArray = result |> Seq.toArray
                match ctx.TryGetHeaderValue HxRequestHeader.Trigger with
                | Some "search" ->
                    return! ctx.WriteHtmlView (index.rows page resultArray)
                | _ ->
                    return! ctx |> writeHtml (index.html search page resultArray archiver)
            | None ->
                let! result = ContactService.all page
                let resultArray = result |> Seq.toArray
                return! ctx |> writeHtml (index.html "" page resultArray archiver)
        }

let getContactsCount: EndpointHandler =
    fun ctx ->
        task {
            let! count = ContactService.count()
            return! ctx.WriteText $"({count} total Contacts)"
        }

let getNewContact: EndpointHandler =
    writeHtml (new'.html ModelState.Empty)

let insertContact: EndpointHandler =
    fun ctx ->
        task {
            match! ctx.BindAndValidateForm<ContactDTO>() with
            | ModelValidationResult.Valid validatedContact ->
                let! _ = validatedContact.ToDomain() |> ContactService.add
                flash "Created new Contact!" ctx
                return ctx.Response.Redirect("/contacts")
            | ModelValidationResult.Invalid invalidModel ->
                return!
                    invalidModel
                    |> ModelState.Invalid
                    |> new'.html
                    |> writeHtml
                    <| ctx
        }

let updateContact id: EndpointHandler =
    fun ctx ->
        task {
            match! ctx.BindAndValidateForm<ContactDTO>() with
            | ModelValidationResult.Valid validatedContact ->
                let domainContact = validatedContact.ToDomain()
                do! ContactService.update({domainContact with Id = id})
                flash "Updated Contact!" ctx
                return ctx.Response.Redirect($"/contacts/{id}")
            | ModelValidationResult.Invalid (contactDto, errors) ->
                return!
                    ({ contactDto with id = id }, errors)
                    |> ModelState.Invalid
                    |> edit.html
                    |> writeHtml
                    <| ctx
        }

let viewContact id: EndpointHandler =
    fun ctx ->
        task {
            let! contact = ContactService.find id
            return! writeHtml (show.html contact) ctx
        }

let getEditContact id: EndpointHandler =
    fun ctx ->
        task {
            let! contact = ContactService.find id
            return!
                contact
                |> ContactDTO.FromDomain
                |> ModelState.Valid
                |> edit.html
                |> writeHtml
                <| ctx
        }

let deleteContact id: EndpointHandler =
    fun ctx ->
        task {
            do! ContactService.delete id
            match ctx.TryGetHeaderValue HxRequestHeader.Trigger with
            | Some "delete-btn" ->
                flash "Deleted Contact!" ctx
                ctx.Response.Redirect("/contacts")
                ctx.SetStatusCode(303)
            | _ ->
                ()
        }

let deleteContacts: EndpointHandler =
    fun ctx ->
        task {
            match ctx.TryGetFormValues "selected_contact_ids" with
            | Some ids ->
                for id in ids do
                    do! id |> int |> ContactService.delete
                flash "Deleted Contacts!" ctx
            | None ->
                ()
            let page = 1
            let! result = ContactService.all page
            let resultArray = result |> Seq.toArray
            return! ctx |> writeHtml (index.html "" page resultArray archiver)
        }

let validateEmail id: EndpointHandler =
    fun ctx ->
        task {
            match ctx.TryGetQueryValue("email") with
            | Some email ->
                let! contact =
                    if id = 0 then
                        Task.FromResult({ Id = 0; First = ""; Last = ""; Phone = ""; Email = email })
                    else
                        ContactService.find id
                let! isValid = ContactService.validateEmail { contact with Email = email  }
                if isValid then
                    return ()
                else
                    return! ctx.WriteText "Invalid email"
            | None ->
                return ()
        }

let startArchive: EndpointHandler =
    fun ctx ->
        task {
            let! contacts = ContactService.getAllContacts()
            archiver <- Archiver(contacts)
            archiver.Run() |> ignore
            return! ctx.WriteHtmlView (index.archiveUi archiver)
        }

let getArchiveStatus: EndpointHandler =
    fun ctx ->
        ctx.WriteHtmlView (index.archiveUi archiver)

let deleteArchive: EndpointHandler =
    fun ctx ->
        archiver.Reset()
        ctx.WriteHtmlView (index.archiveUi archiver)
